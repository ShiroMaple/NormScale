...
function createV2MockChat(overrides: V2MockOverrides = {}): ChatClient {
  const processRules = overrides.processRules ?? [
    ...
    { rule_id: 'PROC_FLARING', category: 'process', property_key: 'flaring', display_name: '扩口试验', rule_type: 'qualitative_and_numeric', requirement_level: 'MANDATORY', criteria: { cone_angle_deg: 60, flaring_rate_min_percent: 18, expected_visual_result: 'NO_CRACKS', test_standard: 'GB/T 242' }, source_clause: '6.5.2.1', applies_to_grades: ['06Cr19Ni10'] },
    ...
    { rule_id: 'SURF_QUALITY', category: 'surface', property_key: 'surface_quality', display_name: '表面外观质量', rule_type: 'qualitative_pass', requirement_level: 'MANDATORY', criteria: { expected: 'CLEAN_PASS' }, source_clause: '6.11', applies_to_grades: 'ALL' },
...
describe('v2 提取范围扩充端到端（mock LLM 覆盖全部任务，默认全量七族）', () => {
  ...
  it('applies_to_grades 臆造牌号：S2 展开未挂载移交 S3 拦截，MANUAL_REVIEW 且不落 staging', async () => {
    ...
    const result = await ingestStandard({
      rawText: V2_FIXTURE_TEXT,
      chatClient: createV2MockChat({
        processRules: [{ rule_id: 'PROC_FLARING', category: 'process', property_key: 'flaring', display_name: '扩口试验', rule_type: 'qualitative_and_numeric', requirement_level: 'MANDATORY', criteria: { cone_angle_deg: 60, flaring_rate_min_percent: 18, expected_visual_result: 'NO_CRACKS' }, source_clause: '6.5.2', applies_to_grades: ['S99999'] }],
      }),
      outRoot,
      cacheRoot,
    });
