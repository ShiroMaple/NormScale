...
5. 【全量无损提取与长尾兜底准则 (Zero-Drop & Fallback Policy)】：
   - 严禁丢弃或遗漏质保书已识别到的任何报审内容与检测事实！单据中的所有表格（包括化学成分表、力学性能表、工艺试验表、尺寸与表面外观清单等）必须逐行逐列完整提取至末尾，严禁提前闭合或截断尾部行；
   - 质保书中声明的任何检验项目、外观质量、几何尺寸、特殊协议条款或供货附加属性，即使在预设核心字段中未找到专门对应的 key，也严禁静默丢弃！必须全量装入 additional_tests 数组中（填写原始项目名 name、实测结论/数值 result 与引用的执行标准 standard），确保原件信息 100% 结构化无损流转，绝不可主观遗漏；
   - 【核心字段互斥与防冗余规范】：凡已提取到化学成分（chemical）、力学性能（mechanical，如抗拉强度、屈服强度、断后伸长率、硬度等）或工艺性能（process）等预设结构中的指标，严禁在 additional_tests 中重复列出！严禁将整个子表格或综合试验方法名（如“室温拉伸试验”、“硬度试验”）以复合字符串形式二次复制塞入 additional_tests。additional_tests 严格仅用于存放未被上述预设字段覆盖的独立长尾/附加检验项（如几何尺寸规格、表面外观质量、交货特殊技术协议要求等）。每个 additional_tests 条目必须原子化（单指标单值），严禁将含多个指标赋值的表格行整段打包为单个条目。
`.trim();
