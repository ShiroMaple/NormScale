...
## 2026-09-04 · 执行标准列重构为“指标主显+来源垂直下沉”排版、横向空间释放与列宽优化

- 执行标准要求列排版彻底重构 (`src/components/WaterfallWorkbench.tsx`):
  1. 响应用户“横向空间更为宝贵、行高允许拉高”指示，彻底淘汰超长单行横向拼接；
  2. 重构为「首行指标主显 + 来源垂直下沉」纵向架构：
     - 一致项：首行加粗显指标（如 `≤ 0.08 % ★`），下方垂直堆叠各标准带年份微型药丸，免去数字重复；
     - 差异/加严项：首行显主导控制指标，下方垂直分行展开各标详细要求与高亮；
  3. 标准列宽由 `min-w-[220px]` 压缩至 `min-w-[170px] w-48`，释放空间给【判定逻辑】(`min-w-[300px]`)；
