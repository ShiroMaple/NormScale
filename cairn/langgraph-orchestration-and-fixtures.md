...
### 3.4 测试用例原件装载与按需种子补齐双模机制 (Dual-Mode Scenario Loading Pipeline)
- **统一文件流水线装载**：前端专测矩阵卡片点击「一键装载」后，前端通过 `fetch(sample.pdf_url)` 获取真实 PDF 二进制 Blob 并实例化为真实 `File` 对象，调用统一的 `handleRealFiles([file])` 压入待处理队列；
- **双模并行与防重保障**：
...
### 4.5 PDF 残缺字体编码与测试数据假自洽的结构性陷阱
- **现象**：前端 PDF 预览视窗出现带叉方框乱码（`☒` Missing Glyph）与文字重叠踩踏；Case 1 人工指定牌号后因缺失执行标准强检项暴雷判定 5 项漏检（FAIL）；
- **根因**：
  1. PDF 生成引擎手写残缺的 Type0 CJK 结构且无 FontDescriptor 与字宽度量，加上客户端 pdf.js 缺少 CMap 映射包，导致 Canvas 栅格化字形丢失回退为豆腐块乱码；
  2. 测试用例原件与数据生成时物理遗漏执行标准强制项（压扁、扩口、晶腐、超声、水压），而在单测文件中通过手写假数据 Mock 字典覆盖掩盖缺陷；
  3. 转换适配层 `specimen-adapter.ts` 硬编码力学键名导致 `ReH` 等别名被截断遗漏。
- **沉淀**：
  1. 采用标准 PDF 1.4 Standard 14 Type 1 矢量生成引擎与 WinAnsiEncoding 工业规范双语排版，配合客户端 `cMapUrl` 配置彻底消除字库缺失与字符踩踏重叠；
  2. 彻底拔除单测中的手写假数据字典，统一从磁盘物理切片读取，实现单测与工作台真实消费同构；
  3. 数据转换层坚持“未知字段全保全”原则，防止长尾或非标力学项在漏斗外侧泄漏。
