...
6. **[已对齐·待实现] 采购技术协议与标准优先级调度体系（Technical Agreement Priority & Override Hierarchy）**：
   - 行业合理性：技术协议属于买卖双方专用要约合同，行业普遍遵循【技术协议 > 行业订货标 > 国家通用标】；
   - 冲突策略：落实分级管控——协议加严直接主导判定，协议放宽/豁免法定底线时高亮预警【合同放宽法标风险】并挂起人工复核；
   - 资产形态：建立【技术协议】专属库（带协议号、品类、有效期），支持工作台直接勾选挂载与随单快速补录加严条款；
   - 优先级链条：默认协议置顶，界面支持拖拽/微调排序权重；
   - 报告与台账：双层主结论分立呈现（法定标准符合性 vs 采购技术协议符合性），兼顾工程交工合规与采购索赔退货。
