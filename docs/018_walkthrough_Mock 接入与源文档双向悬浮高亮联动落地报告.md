...
- **测试用例校验**：[`tests/extractor/zpje-bbox.test.ts`](file:///Users/shiromaple/Github/NormScale/tests/extractor/zpje-bbox.test.ts)
  - 验证 `DEFAULT_INSPECTION_SESSION` 首位为 ZPJE S32168 真实文档，物理切图文件存在且非空；
  - 验证 `getZPJEBBoxes` 像素级重新校准后坐标全部合法，拉伸试验各炉批期望 Y 坐标严格断言通过；
  - 验证全部 19+ 个业务字段 ID 完整映射。
...
| 文件 | 变更说明 |
|---|---|
| [`src/types/bbox.ts`](file:///Users/shiromaple/Github/NormScale/src/types/bbox.ts) | 修改：基于 2379×3366 高清切图像素提取全面重新校准全量 BBox 坐标字典 |
| [`src/components/WaterfallWorkbench.tsx`](file:///Users/shiromaple/Github/NormScale/src/components/WaterfallWorkbench.tsx) | 修改：高亮框改单实线（`border-2 border-primary`），移除 `-top-7` 遮挡标签，在左上角边距呈现定位徽标 |
| [`tests/extractor/zpje-bbox.test.ts`](file:///Users/shiromaple/Github/NormScale/tests/extractor/zpje-bbox.test.ts) | 修改：更新 `expectedTensileY` 像素级 Y 坐标期望值断言 |
| [`cairn/LOG.md`](file:///Users/shiromaple/Github/NormScale/cairn/LOG.md) | 修改：追加单实线重构、零遮挡优化与像素级重校记录 |
