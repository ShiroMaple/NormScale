...
| **8** | **连续重新核验/频繁切标的时序竞态与算力无端开销** | 构建四层纵深防护：批次级 `AbortController` 掐旧启新熔断（$O(1)$ 算力控制）、前端 `runId` 版本令牌校验静默弃包、服务端 `request.signal` 级联熔断、交互层等价置灰与冷却保护。 | **已解决** |
...
### 8. 批次级 AbortController 熔断、版本令牌校验与时序幽灵覆盖彻底根治
- **文件**：[WaterfallWorkbench.tsx](file:///c:/Users/gaoft/Documents/CodeSpace/NormScale/src/components/WaterfallWorkbench.tsx)、[submit/route.ts](file:///c:/Users/gaoft/Documents/CodeSpace/NormScale/src/app/api/audit/submit/route.ts)
- **改动详情**：
  1. 批次级控制器管理：在工作台中维护 `batchAbortControllersRef`，任何新运行（重新核验/重置/切标）调度前，瞬间执行 `prevController.abort('SUPERSEDED_BY_NEW_RUN')` 掐断旧连接；
  2. 版本令牌锁：5 个流式回调中校验 `if (data.taskId && !data.taskId.endsWith(`::${runId}`)) return;`，丢弃迟到的陈旧数据包，彻底消除时序倒挂；
  3. 服务端级联断开：`submit/route.ts` 监听 `request.signal.aborted`，客户端断开时立即退出流式生成器，避免无端大模型调用与 Token 损耗；
  4. 交互层防连击：重置按钮基于 `!isOverridden` 瞬间置灰，「重新核验」按钮增加 500ms 冷却拦截，保障系统在任意时刻仅有 1 个活跃任务（$O(1)$ 算力控制）。
